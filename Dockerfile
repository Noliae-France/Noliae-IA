...
COPY . /app
WORKDIR /app
RUN install -m 0755 /tmp/nolc/nolc /usr/local/bin/nolc && nolc nhtml views/chat.nhtml && nolc build main.nol -o noliae-ia --lien ssl --lien crypto --chemin-lib /usr/lib/x86_64-linux-gnu
FROM ubuntu:24.04
...
COPY --from=build /app/noliae-ia /app/noliae-ia
